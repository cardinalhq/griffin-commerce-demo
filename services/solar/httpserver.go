...
// httpserver.go exposes the demo operator's fault-flipping API. No auth —
// the Service is ClusterIP-only; reach it via `kubectl port-forward`.

const defaultFaultPort = "9999"

func faultPort() string {
	if v := os.Getenv("SOLAR_FAULT_PORT"); v != "" {
		return v
	}
...
func StartHTTPServer(ctx context.Context, sc *Scenario) {
	...
	go func() {
		slog.InfoContext(ctx, "Solar fault HTTP server listening", "addr", addr)
